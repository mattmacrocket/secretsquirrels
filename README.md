...
SecretSquirrels is the orchestration layer for a multi-repository agent workspace. It provides the scripts, configuration, and test harnesses needed to bootstrap, update, and verify runtime repositories, and now also hosts the shared operator control-plane applications.
...
```bash
git clone git@github.com:mattmacrocket/secretsquirrels.git
cd secretsquirrels
...
For the dashboard/API relocation details and cutover steps, see **[docs/controlplane-migration.md](docs/controlplane-migration.md)**.
...
- label: `com.secretsquirrels.controlplane`
- dashboard port: `4317`
- API port: `8199`
- log files: `/tmp/secretsquirrels-controlplane.launchd.log` and `/tmp/secretsquirrels-controlplane.launchd.err.log`
...
SecretSquirrels is source-available under `PolyForm Noncommercial 1.0.0`.
